...
export default function ReviewsPage() {
  return (
    <>
      <Seo
        title="Guest Reviews | Sarayah Phuket Villas"
        description="What guests say about staying at Sarayah's luxury villas in Thalang District, Phuket."
        path="/reviews"
      ...
      <section className="relative bg-primary text-white min-h-[70vh] flex items-end overflow-hidden px-6 sm:px-10 lg:px-20">
        ...
        />
        <div className="relative z-10 max-w-4xl mx-auto lg:mx-0 text-center lg:text-left pt-40 pb-16 sm:pb-20">
          <Breadcrumbs items={CRUMBS} dark className="mb-6 justify-center lg:justify-start" />
          <span className="text-xs uppercase tracking-[0.3em] font-bold text-white/60 mb-4 block">Verified Guest Experiences</span>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-headline mb-6">Guest Reviews</h1>
          <p className="text-white/80 text-lg leading-relaxed">Trusted by families and group travelers alike — discover why Sarayah is the preferred choice for a luxury stay in Thalang.</p>
        </div>
      </section>
      ...
      <section className="py-24 sm:py-32 px-6 sm:px-8 bg-primary text-white text-center">
        <Reveal>
          ...
          <Button variant="tertiary" to="/booking">
            Book Now
          </Button>
        </Reveal>
      </section>
