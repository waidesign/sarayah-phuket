...
import { EXPERIENCES_DATA, EXPERIENCES_LIST } from '../data/experiences';
...
export default function ExperienceDetailPage() {
  ...
  const listEntry = EXPERIENCES_LIST.find((e) => e.slug === slug);
  const crumbs = [
    { name: 'Home', path: '/' },
    { name: 'Experiences', path: '/experiences' },
    { name: listEntry?.title || exp.title, path: `/experiences/${slug}` },
  ];
  ...
  return (
    <>
      <Seo
        ...
        image={exp.heroImage}
        schema={[breadcrumbSchema(crumbs), ...(exp.faqs ? [faqPageSchema(exp.faqs)] : [])]}
      />
      <PageHeader eyebrow={exp.category} title={exp.title} italicTitle={exp.italicTitle} background="brand" breadcrumbs={crumbs} />
      ...
      {exp.body.map((block, i) => (
        <section key={block.title} className={`py-20 sm:py-28 px-6 sm:px-8 ${i % 2 === 0 ? 'bg-surface-container-low' : 'bg-white'}`}>
          <div className={`grid grid-cols-1 ${block.image ? 'lg:grid-cols-2' : ''} gap-12 lg:gap-20 items-center`}>
            <Reveal className={`space-y-6 ${block.image && block.imageSide === 'left' ? 'lg:order-2' : ''}`}>
              ...
              <h2 className="text-3xl sm:text-4xl font-headline text-on-surface leading-tight italic">
                {block.title} <br />
                <span className="font-light not-italic">{block.italicTitle}</span>
              </h2>
      ...
      <section className="py-28 sm:py-36 px-6 sm:px-8 bg-primary text-white text-center">
        <Reveal>
          ...
          <Button
            variant="tertiary"
            size="lg"
            to={ctaExternal ? undefined : ctaHref}
