...
## The "rotated" 2-bedroom photos were never actually broken in a browser

This file previously stated `public/assets/media/villas/2-bedroom/IMG_8217.jpg` and
`IMG_8261.jpg` "display sideways regardless of what CSS is applied" and were avoided
across every page. That claim was re-investigated during a Koray-SEO content pass and
turned out to be **a false positive from a non-EXIF-aware viewer**, not a real bug.

Both files are phone photos: pixel data stored landscape (4032×3024) with an EXIF
`Orientation` tag of `6` ("rotate 90° CW to display correctly") — the normal way phone
cameras save portrait shots. Real browsers (tested directly in Chromium via Playwright)
honor that EXIF tag for plain `<img>` elements and render both photos correctly upright.
They only look sideways in tools that ignore EXIF orientation and show raw pixels
verbatim. A first attempt to "fix" this by physically rotating the pixels with
PowerShell's `System.Drawing` (`RotateFlip` + `Save`) made things *worse*: GDI+'s
`Save()` preserves the original EXIF property items unless you strip them, so the
already-rotated pixels kept the stale `Orientation: 6` tag — causing EXIF-aware
browsers to rotate an already-correct image a second time. This was caught by
screenshotting the result in actual Playwright/Chromium (not by re-reading the file with
a non-EXIF-aware tool, which reported it as fixed when it wasn't) and the images were
reverted to their original, untouched bytes.

**Lesson**: when "fixing" an image orientation problem, verify in a real browser
render, not a raw file preview — some tools honor EXIF orientation, some don't, and
they can disagree about whether the *same bytes* look correct. If the underlying
photo library still bugs you, the safe fix is to `exiftool`-strip the orientation tag
while writing physically-correct pixels in one pass — not a naive `RotateFlip`.
